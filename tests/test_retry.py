...
def async_record_wait_sleep(multiplier=10.0):
    waits, sleeps = [], []
...
def async_clock_advances_on_sleep():
    elapsed = 0.0
    sleeps = []
...
def test_retry_state_machine(
    ...
    result, attempts = retry(fn, n=n, wait=wait, sleep=sleep)
...
def test_invalid_n_validates_before_side_effects(n):
    def fail(*_):
        raise AssertionError("side effect should not run")

    with pytest.raises(ValueError, match="n must be >= 1"):
        retry(fail, n=n, wait=fail, sleep=fail, clock=fail)
...
@pytest.mark.asyncio
async def test_aretry_state_machine(
    outcomes, n, expected, k, reasons, expected_waits, expected_sleeps
):
    fn, fn_calls = async_scripted(outcomes)
    wait, sleep, waits, sleeps = async_record_wait_sleep()

    result, attempts = await aretry(fn, n=n, wait=wait, sleep=sleep)
...
def test_elapsed_tracks_time_spent_sleeping(
    outcomes, n, wait, expected, expected_sleeps
):
    fn, _ = scripted(outcomes)
    clock, sleep, sleeps = clock_advances_on_sleep()

    result, attempts = retry(fn, n=n, wait=wait, sleep=sleep, clock=clock)
...
@pytest.mark.parametrize("outcomes,n,wait,expected,expected_sleeps", ELAPSED_CASES)
@pytest.mark.asyncio
async def test_aretry_elapsed_tracks_time_spent_sleeping(
    outcomes, n, wait, expected, expected_sleeps
):
    fn, _ = async_scripted(outcomes)
    clock, sleep, sleeps = async_clock_advances_on_sleep()

    result, attempts = await aretry(fn, n=n, wait=wait, sleep=sleep, clock=clock)
