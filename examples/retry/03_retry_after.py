...
"""
import random, time
from smolib import retry, Attempt, Ok, Pending, Exhausted, Wait

MAX_SERVER_WAIT = 10.0  # never trust the server more than this

call_count = 0
def fake_api() -> Attempt[str, str, dict]:
    ...
    if call_count <= 3:
        ...
        time.sleep(clamped)
        return Pending(f"429 retry-after={server_retry_after:.1f}")
    return Ok({"data": "here"})

result, attempts = retry(fake_api, n=6, wait=Wait.const(0))
match result:
    case Ok(value=v):
